...
double tsp_mst(point *V, int n, int *P, graph T) {
  ...
  edge *E = (edge*)malloc(n*(n-1)/2*sizeof(edge)); // à remplacer par un malloc(...)
...
int edge_c=0;
  ...
  { 
    E[edge_c].u=i;
    E[edge_c].v=j;
    E[edge_c].weight=dist(V[i],V[j]);
    edge_c++;
  }

   //trier le tableau
  qsort(E,edge_c,sizeof(edge),compEdge);
  ...
  // Initialize degrees of all vertices of T to zero
  for (int i = 0; i < n; i++) {
      T.deg[i] = 0;
  }
  ...
  for(int i=0;i<edge_c;i++)
  {
      ...
      int parent_u=Find(E[i].u,parent);
      int parent_v=Find(E[i].v,parent);
