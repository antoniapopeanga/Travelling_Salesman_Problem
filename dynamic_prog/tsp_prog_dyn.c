  ...
  */
  ...
  for(int S=1;S<C;S++)
  {
    ...
    {  
      ...
      if(DeleteSet(S,t))//la ville est dans l'ensemble
      {
        if(T== 0)//card S=1
        {
        ...
        else
        {
          ...
          for(int x=0;x<n-1;x++)
          {
            if(DeleteSet(S,x)!=T)//x∈S\{t}
            {
              int d=dist(V[x],V[t]);//dist de x a t
              //printf("%f",D[t][DeleteSet(S,x)].length);
              if(D[x][T].length+d<D[t][S].length)
              {
                D[t][S].length=D[x][T].length+d;
                D[t][S].pred=x;//la ville juste avant d'arriver en t
                //printf("card>=1\n");
...
double w = DBL_MAX; // Initialize to infinity

if (running) {
    // Iterate over all cities except the starting city (V[0])
    for (int t = 0; t < L; t++) {
        double d = dist(V[n - 1], V[t]) + dist(V[t], V[0]); // Include the return distance to the starting city
        if (D[t][C - 1].length + d < w) {
            w = D[t][C - 1].length + d;
            ExtractPath(D, t, C - 1, n, Q);
        }
    }
}
...
// Freeing memory
// Freeing the memory allocated for the DP table D
for(int i = 0; i < L; i++) {
    free(D[i]);
}
free(D);

return w; // Returning the length of the optimal tour
